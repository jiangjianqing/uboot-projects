...
#include <string.h>
#include <stdlib.h>
#define BUFSIZE    (512*1024)   ////u-boot.bin max size
#define IMG_SIZE   (16*1024)	//s5pv210 read 16k to bl0
...
int main (int argc, char *argv[])
{
    ...
    char *Buf, *a;
    int BufLen;
    int nbytes, fileLen , new_fileLen;
	...
	printf("nbytes = %d , fileLen=%d , new_fileLen=%d \n",nbytes,fileLen,new_fileLen);
    if ( nbytes != fileLen ) {
    ...
    *( (unsigned int *)a ) = checksum;
    ...
    nbytes = fwrite( a, 1, new_fileLen, fp);
    if ( nbytes !=  new_fileLen) {
